...
class TestAbstractHandler(unittest.TestCase):
    ...
    def test_add_resource_driver(self):
        ...
        # verify
        self.cs_session.UpdateResourceDriver.assert_called_once_with(
    ...
    def test_create_cs_resource(
        ...
        # act
        result = self.tested_instance._create_cs_resource(
            cs_session=self.cs_session,
            resource_name=resource_name,
        ...
        self.cs_session.CreateResource.assert_called_once_with(
    ...
    def test_create_cs_resource_resource_name_is_taken(self):
        ...
        self.cs_session.CreateResource.side_effect = [
            CloudShellAPIError(
                code=CloudshellAPIErrorCodes.RESOURCE_ALREADY_EXISTS,
                message="",
                rawxml="",
            ),
            None,
        ...
        self.cs_session.CreateResource.assert_any_call(
            resourceFamily=resource_family,
            resourceModel=resource_model,
            resourceName=resource_name,
            resourceAddress=device_ip,
    ...
    def test_get_cli_credentials(self, telnet_session_class, ssh_session_class):
        ...
        vendor_settings = mock.MagicMock(
            get_creds_by_vendor=mock.MagicMock(return_value=vendor_cli_creds)
        )
        device_ip = "device_ip"
        ...
        # act
        self.tested_instance._get_cli_credentials(
            vendor=vendor, vendor_settings=vendor_settings, device_ip=device_ip
        )
        # verify
        telnet_session_class.assert_called_once_with(device_ip)
        ssh_session_class.assert_called_once_with(device_ip)

        ssh_session.check_credentials.assert_called_once_with(
    ...
    def test_get_cli_credentials_handles_exception(
        ...
        telnet_session.check_credentials.assert_called_once_with(
            cli_credentials=vendor_cli_creds,
            default_prompt=vendor.default_prompt,
            enable_prompt=vendor.enable_prompt,
            logger=self.logger,
