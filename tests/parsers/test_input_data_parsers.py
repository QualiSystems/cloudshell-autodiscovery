...
from autodiscovery.parsers.input_data_parsers import (
    AbstractInputDataParser,
    JSONInputDataParser,
    YAMLInputDataParser,
    get_input_data_parser,
)
...
class TestAbstractInputDataParser(unittest.TestCase):
    ...
    def test_parse_devices_ips(self, device_ip_range_class):
        ...
        self.tested_instance._find_ips = mock.MagicMock(
            side_effect=[first_parsed_ips, second_parsed_ips]
        )
        ...
        devices_ips = [
            {"range": "192.168.10.3-45", "domain": "Test Domain"},
        ...
        # verify
        self.assertEqual(result, expected_res)
        device_ip_range_class.assert_any_call(
            ip_range=first_parsed_ips, domain="Test Domain"
        )
