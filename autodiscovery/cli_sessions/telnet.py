...
from cloudshell.cli.session.telnet_session import TelnetSession, TelnetSessionException
...
class TelnetDiscoverySession(TelnetSession, AbstractDiscoverySession):
    ...
    def check_credentials(self, cli_credentials, default_prompt, enable_prompt, logger):
        ...
        try:
            ...
            re_prompts = "|".join(
                [prompt for prompt in (default_prompt, enable_prompt) if prompt]
            )
            output_str = self.hardware_expect(
                None,
            ...
            return self._check_enable_password(
