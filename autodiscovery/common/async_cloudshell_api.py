...
from cloudshell.api.common_cloudshell_api import CommonAPISession, CloudShellAPIError, XMLWrapper, CommonApiResult, \
    CommonAPIRequest
...
class AsyncCloudShellAPISession(CloudShellAPISession):
    ...
    async def _sendRequest(self, username, domain, operation, message):
        ...
        request_headers['Authorization'] = f"MachineName={self.hostname};Token={self.token_id}"
    ...
    async def generateAPIRequest(self, kwargs):
        ...
        response_str = await self._sendRequest(self.username, self.domain, method_name,
                                               etree.tostring(request_node).decode("utf-8"))

        response_str = response_str.replace('xmlns="http://schemas.qualisystems.com/ResourceManagement/'
                                            'ApiCommandResult.xsd"', '').replace('&#x0;', '<NUL>')
    ...
    async def Logon(self, username='', password='', domainName='Global'):
        return await self.generateAPIRequest(OrderedDict(
            [('method_name', 'Logon'), ('username', username), ('password', password), ('domainName', domainName)]))

    async def SecureLogon(self, token='', domainName='Global'):
        return await self.generateAPIRequest(OrderedDict(
            [('method_name', 'SecureLogon'), ('token', token), ('domainName', domainName)]))

    async def CreateResource(self, resourceFamily='', resourceModel='', resourceName='', resourceAddress='',
