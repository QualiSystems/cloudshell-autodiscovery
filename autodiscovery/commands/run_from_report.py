    ...
    def execute(self, additional_vendors_data):
        ...
        for entry in self.report.entries:
            ...
            try:
                ...
                self.output.send("Failed to discover {} device. {}".format(entry.ip,
                                                                           entry.comment), error=True)
                self.logger.exception("Failed to upload {} device due to:".format(entry.ip))
            else:
                self.output.send("Device with IP {} was successfully uploaded".format(entry.ip))
                self.logger.info("Device with IP {} was successfully uploaded".format(entry.ip))
