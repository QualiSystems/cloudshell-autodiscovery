...
@cli.command(name="echo-excel-connections-template")
...
def run_from_report(input_file, config_file, log_file, report_file, autoload):
    ...
    report = reports.discovery.get_report(report_file=report_file, report_type=reports.discovery.DEFAULT_REPORT_TYPE)
...
@click.option("--input-file", required=True, help="Input file with CloudShell configuration data. "
                                                  "Can be generated with a 'echo-input-template' command")
@click.option("--connections-file", required=True, help="Connections file with ports data. Can be generated with a "
                                         "'echo-excel-connections-template' command")
@click.option("--report-file", required=True, help="File name of the report to run from")
@click.option("--log-file", help="File name for logs")
# @click.option("--report-file", required=True, help="File name of the report to run from")
def connect_ports(input_file, connections_file, report_file, log_file):
    ...
    # todo: stub for now
    parsed_entries = [("port1 name", "port2 name", "connect")]

    # report = reports.get_report(report_file=report_file, report_type=reports.discovery.DEFAULT_REPORT_TYPE)
    # parsed_entries = report.parse_entries_from_file(report_file)

    command = commands.ConnectPortsCommand(report=reports.connections.get_report(report_file=report_file,
                                                                                 report_type=reports.discovery.DEFAULT_REPORT_TYPE),
                                           output=ConsoleOutput())

    command = commands.ConnectPortsCommand(output=ConsoleOutput())
